...
CorrinoMainTankTypes = {
	EarlyGame = { "combat_tank_c" },
	LateGame = { "combat_tank_c", "combat_tank_c", "siege_tank", "missile_tank" }
}
...
ActivateAI = function()
	...
	Trigger.AfterDelay(InitialProductionDelay["AtreidesMain"][Difficulty], function ()
		...
		ProduceUnits(AtreidesMain, AHiTechFactory, delay, aircraftToBuild, AttackGroupSize[Difficulty], AttackThresholdSize)
	end)
	...
	local productionTypesAtreides =
	{
		...
		high_tech_factory = aircraftToBuild
	}
	...
	Trigger.OnBuildingPlaced(AtreidesMain, function(p, building)
		...
		ProduceUnits(AtreidesMain, building, delay, productionTypesAtreides[building.Type], AttackGroupSize[Difficulty], attackThresholdSize)
	end)
